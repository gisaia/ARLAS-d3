...
usage(){
	...
	echo " -version     arlas-d3 version release,level of evolution"
...
releaseProd(){
    ...
    echo "=> Build the ARLAS-d3 library"
    npm install
    npm run lint
    ...
    docker run --rm -v "$(pwd)":/usr/local/src/your-app gisaia/github-changelog-generator:latest github_changelog_generator \
      -u gisaia -p ARLAS-d3 --token ${GITHUB_CHANGELOG_TOKEN} --no-pr-wo-labels --no-issues-wo-labels --no-compare-link --no-unreleased \
    ...
    if [ "$STAGE_LOCAL" == "stable" ] || [ "$STAGE_LOCAL" == "rc" ];
        then
        send_chat_message "Release of arlas-d3, version ${VERSION}"
    fi
...
if [ ! -z ${RELEASE_VERSION+x} ];
    then
        echo "Release ARLAS-d3 version                    : ${RELEASE_VERSION}";
